...
class CacheMethodsSpec extends Specification {
    ...
    def "validUnit sets expires header"() {
        ...
        then:
        responseHeaderToHTTPDate(testController.response.getHeader('Expires')) == dateToHTTPDate(testController.request.getAttribute('test_validUntil'))
    }
    ...
    private static String responseHeaderToHTTPDate(h) {
        return dateToHTTPDate(new Date(h as long))
    }
}